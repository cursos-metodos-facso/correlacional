...
Para contrastar esta hipótesis, vamos a hacer un subset de datos de la CASEN 2022 de 1.500 casos.
...
La prueba T que evalúa la diferencia de medias entre el salario y el sexo sugiere que el efecto es positivo y estadísticamente signficativo (diferencia = 32567.11, t(566.69) = 2.05, p < .05). Por tanto, con un 95% de confianza, rechazamos la $H_{0}$ ya que existe evidencia a favor de nuestra $H_{A}$ respecto a que el salario de los hombres es mayor al salario de las mujeres.

Además, podemos calcular un intervalo de confianza que acompaña nuestra estimación. En este caso, el IC del 95% es (59.670, Inf), donde “Inf” representa infinito. Esto significa que podemos estar 95% seguros de que _la diferencia real entre las medias está por encima de 59.670_. En otras palabras, con alta confianza, podemos afirmar que la media del “grupo 1” es significativamente mayor que la del “grupo 2”.
...
goldin_data <- proc_casen %>% 
  select(ocupado, sexo, ytrabajocor, hijo) %>% 
...
El primer paso es establecer el tipo de hipótesis a formular. ¿Direccional o no direccional? Si es direccional, ¿cuál es el sentido? 

**Pasos 2, 3 y 4 de una vez con R**

Resuelve con trabajo en software.
...
Prodia ser algo con nivel eduacional (var educ en proc_casen)
