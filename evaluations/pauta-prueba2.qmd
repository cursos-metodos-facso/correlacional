...
## Forma E (data_recuperativa: `pref_redis` y `universitaria`)

1.1 Código de correlación y gráfico de dispersión (1p)

```{r}
load(url("https://github.com/cursos-metodos-facso/datos-ejemplos/raw/refs/heads/main/data_recuperativa.RData"))
```

```{r}
cor.test(data_recuperativa$ingresos,
         data_recuperativa$pref_redis,
         complete.obs = TRUE,
         method = "pearson")

sjPlot::plot_scatter(data = data_recuperativa,
                     x = ingresos,
                     y = pref_redis)

```

* 0.5 pts por el código de cor.test(...)

* 0.5 pts por el código de plot_scatter(...)

1.2 Justificación del coeficiente e interpretación (3p)

  * 1pt por coeficiente de correlación seleccionado
  * 1pt por inferencia
  * 1pt por magnitud y sentido del efecto

Se empleó el coeficiente de correlación de Pearson debido a la naturaleza de las variables, en tanto `ingresos` y `pref_redis` corresponden a variables numéricas de razón/intervalo, adecuadas para este tipo de correlación.

El coeficiente da cuenta de una relación negativa y de magnitud moderada siguiendo los criterios de Cohen (1988) (por ejemplo, un valor de r alrededor de -0.46). Es decir, a medida que aumentan los ingresos, también tienden a disminuir las preferencias redistributivas (pref_redis).

La relación es estadísticamente significativa (p < 0.001), por lo que es posible rechazar H₀ de no asociación entre ambas variables, entregando evidencia a favor de una relación entre ingresos y pref_redis.

2.1 Matriz de correlaciones (1p)

```{r}
cormat <- data_recuperativa %>% 
  dplyr::select(universitaria, pref_redis, ingresos)
...
(0,5 por cada correlación significativa)

* Ingresos – pref_redis: correlación negativa y estadísticamente significativa (p < 0.01). Se rechaza H₀ con 99.9% de confianza correspondiente

* Universitaria – pref_redis: correlación positiva, y estadísticamente significativa (p < 0.01). Se rechaza H₀ con 99.9% de confianza correspondiente
...
2.2c ¿Cómo se denomina el tipo de correlación que se calcula entre educación e ingreso? (1p)

- La correlación entre la variable dummy educacion (0="no universitaria"; 1="universitaria o más") y la variable continúa ingresos se denomina correlación punto biserial.
...
chisq.test(data_recuperativa$universitaria,
           data_recuperativa$ingresos_rec)  # adicional, ya que el Chi2 aparece en sjtab

```

3.2 Interprete el Chi2 en términos de inferencia y magnitud del efecto. (3p)

El test Chi-cuadrado entre universitaria e ingresos_rec **no es estadísticamente significativo** (p > 0.05), por lo que no se puede rechazar la hipótesis nula de independencia entre ambas variables con el nivel de confianza correspondiente.

El tamaño del efecto, medido con la V de Cramer, es de magnitud pequeña (0.059).

Interpretación sustantiva: a medida que se pasa de los tramos de bajos a altos ingresos, no se modifica la proporción de personas con educación universitaria, lo que no refleja una asociación consistente entre nivel socioeconómico y logro educacional.

3.3 En el contexto de una tabla de contingencia, el test Chi-cuadrado de independencia se utiliza para evaluar si existe asociación entre dos variables categóricas. ¿Cuál es la hipótesis nula de este test y qué significa, en términos sustantivos, que el resultado sea estadísticamente significativo? (1p)

La hipótesis nula del test Chi-cuadrado de independencia plantea que no existe asociación entre las dos variables categóricas; es decir, que son independientes y la distribución de una variable no cambia según las categorías de la otra.

Un resultado estadísticamente significativo (p < 0.05) implica que las diferencias entre las frecuencias observadas y las esperadas bajo independencia son demasiado grandes como para atribuirlas al azar, por lo que se rechaza la hipótesis nula y se concluye que hay evidencia de asociación entre las variables.